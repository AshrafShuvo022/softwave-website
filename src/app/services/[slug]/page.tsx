    ...
    <div className="bg-[#0d0d0d]">
              ...
              <ul className="space-y-4">
                {service.offerings.map((offering) => (
                  <li key={offering} className="flex items-start gap-3">
                    ...
                    <span className="text-[#9ca3af]">{offering}</span>
                  </li>
                ))}
              </ul>
            </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-3">
                {service.technologies.map((tech) => (
                  <span
                    key={tech}
                    className="px-3 py-1.5 rounded-lg bg-[#1a1a1a] border border-[#3d3d3d]/50 text-[#9ca3af] text-sm"
                  >
                    {tech}
                  </span>
                ))}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {service.process.map((p, i) => (
              <div key={p.step} className="p-6 rounded-2xl bg-[#0d0d0d] border border-[#3d3d3d]/50">
                <div className="text-4xl font-bold text-[#e8735f]/20 mb-4">0{i + 1}</div>
                <h3 className="text-lg font-bold text-[#faf4f1] mb-2">{p.step}</h3>
      ...
      <section className="py-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-4xl font-bold text-[#faf4f1] mb-4">Ready to Start a Project?</h2>
          <p className="text-[#9ca3af] text-lg mb-8">Let&apos;s talk about what we can build together.</p>
          <Link
            href="/contact"
